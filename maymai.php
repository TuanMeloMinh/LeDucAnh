...
<body>
  ...
  $item_per_page = !empty($_GET['per_page']) ? $_GET['per_page'] : 8;
        ...
        if ($search) {
          ...
          $products = mysqli_query($con, "SELECT * FROM `product` where loai = 'MÁY MÀI' ORDER BY `id` ASC  LIMIT " . $item_per_page . " OFFSET " . $offset);
          $totalRecords = mysqli_query($con, "SELECT * FROM `product` where loai = 'MÁY MÀI'");
        }
