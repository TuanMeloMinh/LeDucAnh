                ...
                if (isset($_POST['name']) && !empty($_POST['name']) && isset($_POST['price']) && !empty($_POST['price'])) {
                    ...
                    if (!isset($error)) {
                        if ($_GET['action'] == 'edit' && !empty($_GET['id'])) {
                            $result = mysqli_query($con, "UPDATE `product` SET `name` = '" . $_POST['name'] . "', `quantity` = '".$_POST['quantity']."' ,`image` =  '" . $image . "', `price` = " . str_replace('.', '', $_POST['price']) . ", `loai` = '" . $_POST['loai'] . "', `mieuta` = '" . $_POST['mieuta'] . "', `last_updated` = " . time() . " WHERE `product`.`id` = " . $_GET['id']);
                        } else { 
                            $result = mysqli_query($con, "INSERT INTO `product` (`id`, `name`, `quantity`, `image`, `price`, `loai`, `mieuta`, `created_time`, `last_updated`) VALUES (NULL, '" . $_POST['name'] . "', '" . $_POST['quantity'] . "' ,'" . $image . "', " . str_replace('.', '', $_POST['price']) . ",'" . $_POST['loai'] . "', '" . $_POST['mieuta'] . ",0,', " . time() . ", " . time() . ");");
                        }
                        if (!$result) { 
